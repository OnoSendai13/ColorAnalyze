...
function chooseOptimalK(pixels, minK, maxK) {
  ...
  /** Exported utilities for weights and disruption */
  export function computeWeights(colors) {
    if (!Array.isArray(colors) || colors.length === 0) return [];
    const total = colors.reduce((s, c) => s + (c.count || 0), 0) || 1;
    return colors.map(c => ({
      ...c,
      weight: (c.count || 0) / total
    }));
  }

  export function disruptionScore(colors, harmonyType = 'complementary') {
    if (!Array.isArray(colors) || colors.length < 2) return 0;
    const total = colors.reduce((s,c)=>s+(c.count||0),0)||1;
    const weights = colors.map(c => (c.count||0)/total);
    let score = 0;
    for (let i=0;i<colors.length;i++) {
      for (let j=i+1;j<colors.length;j++) {
        const h1 = colors[i].hsl?.h ?? colors[i].h ?? 0;
        const h2 = colors[j].hsl?.h ?? colors[j].h ?? 0;
        const d = Math.min(Math.abs(h1-h2), 360-Math.abs(h1-h2));
        const ideal = harmonyType==='complementary'?180:60;
        const dev = Math.abs(d - ideal);
        score += weights[i]*weights[j]*dev;
      }
    }
    return score;
  }
