...
/** RGB -> RYB approximatif */
export function rgbToRyb({ r, g, b }) {
  // conversion simple via CMY intermédiaire, ponytail: suffisante pour comparaison relative
  const c = rgbToCmy({ r, g, b });
  return {
    r: Math.round(c.c * 0.8 + c.m * 0.2),
    y: Math.round(c.y * 0.7 + c.m * 0.3),
    b: Math.round(c.c * 0.2 + c.m * 0.5 + c.y * 0.3),
  };
}
